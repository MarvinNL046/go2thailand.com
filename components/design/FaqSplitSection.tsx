...
export function FaqSplitSection({ id = 'vragen', eyebrow, title, description, items, openFirst = true }: FaqSplitSectionProps) {
  return (
    <section id={id} className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
      <div className="container-custom grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
        ...
        <div className="divide-y divide-jade/10 border-y border-jade/10">
          {items.map((item, index) => (
            <details key={item.question} className="group py-1" open={openFirst && index === 0}>
              ...
              <p className="max-w-[820px] pb-6 pr-10 text-sm font-medium leading-7 text-charcoal/78">{item.answer}</p>
            </details>
          ))}
        </div>
      </div>
