...
export interface DishEditorialData {
  ...
  affiliate: { href: string; title: string; text: string };
  classHref: string;
  classCopy: string;
    ...
    <>
      ...
      <div className="overflow-hidden bg-canvas text-charcoal" data-premium-template="dish-editorial">
        ...
        <section id="cook" className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
          <div className="container-custom grid gap-6 lg:grid-cols-[1.2fr_.8fr]"><article className="rounded-[28px] bg-jade p-8 text-white shadow-editorial-lift sm:p-10"><p className="eyebrow !text-saffron-light">Cook with a sequence</p><h2 className="font-display text-[3.2rem] font-semibold leading-[.88]">{data.cooking.title}</h2><p className="mt-6 max-w-2xl text-sm font-medium leading-7 text-white/67">{data.cooking.intro}</p><ol className="mt-8 grid gap-3 sm:grid-cols-2">{data.cooking.steps.map((step, index) => <li key={step} className="flex gap-3 rounded-xl border border-white/10 bg-white/[.05] p-4 text-xs font-semibold leading-6 text-white/72"><span className="font-black text-saffron-light">{index + 1}</span>{step}</li>)}</ol><p className="mt-6 border-t border-white/12 pt-5 text-[10px] font-medium leading-5 text-white/48">{data.cooking.boundary}</p></article><aside className="rounded-[28px] border border-jade/10 bg-white p-8 shadow-editorial-card"><BookOpen className="text-jade" /><p className="eyebrow mt-6">One useful home reference</p><h2 className="font-display text-[2.4rem] font-semibold leading-none text-jade">{data.affiliate.title}</h2><p className="mt-5 text-sm font-medium leading-7 text-charcoal/64">{data.affiliate.text}</p><a href={data.affiliate.href} target="_blank" rel="noopener noreferrer nofollow sponsored" className="btn-jade btn-jade-pattern mt-6">Check current price at Amazon <ExternalLink size={14} className="text-saffron" /></a><AffiliateDisclosure className="mt-4">Amazon affiliate link through our central `/go/` route. As an Amazon Associate we earn from qualifying purchases. OneLink may send you to a local store; price, seller and availability vary by country.</AffiliateDisclosure></aside></div>
        </section>

        <section className="section-divider-bottom bg-tonal py-14 lg:py-20"><div className="container-custom grid gap-7 lg:grid-cols-[.78fr_1.22fr] lg:items-center"><div><p className="eyebrow">Learn through the wok</p><h2 className="font-display text-[3rem] font-semibold leading-[.9] text-jade">A class can answer what a recipe cannot.</h2><p className="mt-5 text-sm font-medium leading-7 text-charcoal/64">{data.classCopy}</p><a href={data.classHref} target="_blank" rel="noopener noreferrer nofollow sponsored" className="btn-jade btn-jade-pattern mt-6">Check current cooking classes <ExternalLink size={14} className="text-saffron" /></a><AffiliateDisclosure className="mt-4">Klook affiliate link. Check the actual menu, city, language, dietary support, group size and cancellation terms before booking.</AffiliateDisclosure></div><div className="grid gap-px overflow-hidden rounded-2xl border border-jade/10 bg-jade/10 sm:grid-cols-3">{[[ChefHat,'Technique','Watch noodle hydration, sauce reduction and heat.'],[UtensilsCrossed,'Ingredients','Identify tamarind, dried shrimp, tofu and finishing garnishes.'],[ShoppingBasket,'Market context','Learn what can be substituted and what changes the dish.']].map(([Icon,title,text])=>{const ItemIcon=Icon as LucideIcon; return <article key={title as string} className="bg-white p-6"><ItemIcon className="text-jade" /><h3 className="mt-5 font-display text-[1.6rem] font-semibold text-jade">{title as string}</h3><p className="mt-3 text-xs font-medium leading-6 text-charcoal/58">{text as string}</p></article>})}</div></div></section>
