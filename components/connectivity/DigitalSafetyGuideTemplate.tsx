  ...
  return (
    <div className="bg-canvas" data-premium-template="digital-safety-guide-en">
      ...
      <EditorialHero
        ...
        actions={[
          { label: 'See when a VPN helps', href: '#decision', kind: 'primary' },
          { label: 'Check the current NordVPN plan', href: data.vpnHref, kind: 'secondary', newTab: true, affiliate: true },
        ]}
        disclosure="The NordVPN link is sponsored. We may earn a commission at no extra cost to you. Check the current plan, renewal terms, supported devices and refund conditions before buying."
        ...
        contentTone="light"
      />
      <PageSectionNav label="On this Thailand VPN guide" items={data.navItems} />

      <section id="decision" className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
        <div className="container-custom">
          <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><SectionHeading eyebrow="Start with the connection" title={<>Do you need<br />a VPN in Thailand?</>} /><p className="max-w-[760px] text-sm font-medium leading-7 text-charcoal/64">Not for every tap. A VPN is most useful on shared or untrusted Wi-Fi, for a company requirement, or when you deliberately need a different network location. It is not a licence, antivirus product or invisibility cloak.</p></div>
      ...
      <section id="scope" className="section-divider-bottom scroll-mt-24 bg-tonal py-14 lg:py-20">
        <div className="container-custom grid gap-8 lg:grid-cols-2">
          <article className="rounded-[26px] border border-jade/10 bg-white p-7 shadow-editorial-card lg:p-9">
            <p className="eyebrow">What the tunnel can protect</p>
            <h2 className="font-display text-[2.7rem] font-semibold leading-[.92] text-jade">One connection.<br />Between two endpoints.</h2>
          ...
          <article className="relative overflow-hidden rounded-[26px] bg-jade p-7 text-white shadow-editorial-card lg:p-9">
            ...
            <p className="eyebrow !text-saffron-light">What it cannot promise</p>
            <h2 className="font-display text-[2.7rem] font-semibold leading-[.92]">Privacy is not<br />anonymity.</h2>
      ...
      <section id="networks" className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
        <div className="container-custom">
          <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><SectionHeading eyebrow="Door-to-door digital safety" title={<>Match the tool<br />to the network.</>} /><p className="max-w-[760px] text-sm font-medium leading-7 text-charcoal/64">There is no honest universal “high-risk café” label. Use observable signals: who operates the network, whether it is the correct network, what you plan to do and whether your device is updated.</p></div>
          <div className="mt-10 overflow-x-auto rounded-2xl border border-jade/10 bg-white shadow-editorial-card"><table className="w-full min-w-[900px] border-collapse text-left"><thead className="bg-jade text-[9px] font-extrabold uppercase tracking-[.14em] text-saffron-light"><tr><th className="p-5">Connection</th><th className="p-5">First move</th><th className="p-5">VPN role</th><th className="p-5">Watch</th></tr></thead><tbody>{data.contexts.map(({ network, firstMove, vpnRole, watch, icon: Icon }) => <tr key={network} className="border-t border-jade/10 align-top"><th scope="row" className="p-5"><span className="flex items-center gap-3 font-display text-[1.3rem] font-semibold text-jade"><Icon size={20} />{network}</span></th><td className="p-5 text-xs font-medium leading-6 text-charcoal/62">{firstMove}</td><td className="p-5 text-xs font-extrabold leading-6 text-jade">{vpnRole}</td><td className="p-5 text-xs font-medium leading-6 text-charcoal/62">{watch}</td></tr>)}</tbody></table></div>
        </div>
      </section>
      ...
      <section id="law" className="section-divider-bottom scroll-mt-24 bg-jade py-14 text-white lg:py-20">
        <div className="container-custom grid gap-10 lg:grid-cols-[.68fr_1.32fr] lg:items-center">
          <div><p className="eyebrow !text-saffron-light">Software versus behaviour</p><h2 className="font-display text-[3.2rem] font-semibold leading-[.9]">A VPN does not<br />rewrite Thai law.</h2></div>
          <div className="rounded-[24px] border border-white/14 bg-white/[.06] p-7 lg:p-9"><p className="font-display text-[1.75rem] font-semibold leading-tight">The official Thai computer-law sources checked for this guide regulate computer-related conduct and content; they do not give a traveller blanket permission or immunity because traffic uses a VPN.</p><p className="mt-5 text-sm font-medium leading-7 text-white/64">We therefore avoid an absolute legality badge. VPN software is widely offered and commonly used, but the underlying action, content, platform terms and any work-policy rules still apply. This is practical travel information, not legal advice. Check current official guidance if your use case is sensitive.</p><Link href="/practical-info/etiquette-culture/" className="mt-6 inline-flex items-center gap-2 text-xs font-extrabold text-saffron-light">Read the Thailand law-and-etiquette context <ArrowRight size={13} /></Link></div>
        </div>
      </section>

      <section id="choose" className="section-divider-bottom scroll-mt-24 bg-tonal py-14 lg:py-20">
        <div className="container-custom"><div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><SectionHeading eyebrow="Choose by evidence" title={<>Six checks before<br />you subscribe.</>} /><p className="max-w-[760px] text-sm font-medium leading-7 text-charcoal/64">“Fastest” and “best” change by network, server, device and test method. Compare capabilities you can verify for your own trip instead of treating a sponsored ranking as a permanent fact.</p></div><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{data.selectionCriteria.map(({ title, copy, proof, icon: Icon }) => <article key={title} className="flex min-h-[280px] flex-col rounded-2xl border border-jade/10 bg-white p-6 shadow-editorial-card"><Icon size={24} className="text-jade" /><h3 className="mt-6 font-display text-[1.55rem] font-semibold text-jade">{title}</h3><p className="mt-3 text-xs font-medium leading-6 text-charcoal/62">{copy}</p><p className="mt-auto border-t border-jade/10 pt-5 text-[10px] font-extrabold leading-5 text-saffron-dark">Verify: {proof}</p></article>)}</div></div>
      </section>
      ...
      <section id="layers" className="section-divider-bottom scroll-mt-24 bg-tonal py-14 lg:py-20">
        <div className="container-custom"><div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><SectionHeading eyebrow="Three separate layers" title={<>Connection.<br />Tunnel. Account.</>} /><p className="max-w-[760px] text-sm font-medium leading-7 text-charcoal/64">An eSIM gets the device online. A VPN can tunnel network traffic. Account security still depends on updates, unique credentials and multi-factor authentication. Buying one does not replace the others.</p></div><div className="mt-10 grid gap-5 lg:grid-cols-3">{data.layers.map(({ number, title, copy, href, icon: Icon }) => <article key={number} className="rounded-[22px] border border-jade/10 bg-white p-7 shadow-editorial-card"><div className="flex items-center justify-between"><Icon size={25} className="text-jade" /><span className="font-display text-[2.2rem] font-semibold text-jade/16">{number}</span></div><h3 className="mt-7 font-display text-[1.8rem] font-semibold text-jade">{title}</h3><p className="mt-4 text-xs font-medium leading-6 text-charcoal/62">{copy}</p>{href && <Link href={href} className="mt-6 inline-flex items-center gap-2 text-xs font-extrabold text-jade">Open the specialist guide <ArrowRight size={13} /></Link>}</article>)}</div></div>
      </section>

      <section id="next" className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
        <div className="container-custom"><div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end"><SectionHeading eyebrow="Current next steps" title={<>Choose the layer<br />you actually need.</>} /><p className="max-w-[760px] text-sm font-medium leading-7 text-charcoal/64">Check live terms rather than relying on an old monthly price. The sponsored option is clearly labelled; the editorial criteria above apply to any provider.</p></div><div className="mt-10 grid gap-4 md:grid-cols-3">{data.nextSteps.map(({ title, copy, href, label, icon: Icon, affiliate }) => <article key={title} className="flex min-h-[285px] flex-col rounded-2xl border border-jade/10 bg-white p-7 shadow-editorial-card"><Icon size={25} className="text-jade" /><h3 className="mt-6 font-display text-[1.7rem] font-semibold text-jade">{title}</h3><p className="mt-4 flex-1 text-xs font-medium leading-6 text-charcoal/62">{copy}</p>{affiliate ? <a href={href} target="_blank" rel="noopener noreferrer nofollow sponsored" className="mt-6 inline-flex items-center gap-2 text-xs font-extrabold text-saffron-dark">{label} <ExternalLink size={13} /></a> : <Link href={href} className="mt-6 inline-flex items-center gap-2 text-xs font-extrabold text-jade">{label} <ArrowRight size={13} /></Link>}</article>)}</div><AffiliateDisclosure className="mt-4">Sponsored VPN and eSIM links show current commercial offers. Go2Thailand does not set the plan, renewal, coverage, server availability, device support, price or refund conditions.</AffiliateDisclosure></div>
      </section>
