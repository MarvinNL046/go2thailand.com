...
export function DestinationGuideTemplate({ data }: DestinationGuideTemplateProps) {
  ...
  const stayGuideHref = data.stayGuideHref === null ? null : normalizeNlInternalHref(data.stayGuideHref || `/best-hotels/${data.citySlug}/`);
  const foodGuideHref = data.foodGuideHref === null ? null : normalizeNlInternalHref(data.foodGuideHref || `/city/${data.citySlug}/food/`);
  const relatedGuides = data.relatedGuides.map(guide => ({ ...guide, href: normalizeNlInternalHref(guide.href) }));
      ...
      <div className="bg-canvas text-charcoal">
        ...
        <section id="zones" className="section-divider-bottom scroll-mt-24 bg-tonal py-14 lg:py-20">
          <div className="container-custom">
            ...
            <div className="mt-9 grid gap-5 lg:grid-cols-12">{data.zones.map((zone, index) => <ZoneCard key={zone.slug} citySlug={data.citySlug} stayGuideHref={stayGuideHref} zone={zone} index={index} zoneCount={data.zones.length} />)}</div>
          </div>
        </section>
        ...
        <section className="section-divider-bottom py-14 lg:py-20">
          <div className="container-custom relative min-h-[390px] overflow-hidden rounded-2xl bg-jade-dark shadow-[0_16px_45px_rgba(18,63,54,0.16)]">
            ...
            <div className="relative z-10 flex min-h-[390px] max-w-[560px] flex-col justify-center p-7 text-white sm:p-10 lg:p-12">
              ...
              <div className="mt-7 flex flex-wrap gap-3">{stayGuideHref ? <Link href={stayGuideHref} className="btn-cream">Vergelijk de gebieden <ArrowRight size={14} className="text-saffron-dark" /></Link> : null}<a href={tripHref} target="_blank" rel="noopener noreferrer nofollow sponsored" className="inline-flex min-h-12 items-center gap-2 rounded-lg border border-white/30 px-5 text-xs font-extrabold text-white transition hover:bg-white/10">Bekijk hotels <ExternalLink size={14} /></a></div>
        ...
        <RelatedGuidesSection title={`Bouw je ${data.cityName}-reis verder uit`} guides={relatedGuides} sideLink={{ label: 'Uitjes via Klook', href: klookHref, affiliate: true }} disclosure="Klook is een affiliatepartner. Een eventuele commissie verandert jouw prijs niet." />
