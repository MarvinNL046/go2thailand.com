        ...
        <section id="doen" className="section-divider-bottom scroll-mt-24 py-14 lg:py-20">
          <div className="container-custom">
            ...
            <div className="mt-9 grid gap-5 lg:grid-cols-3">
              {data.highlights.map((highlight) => <Link key={highlight.title} href={highlight.href} className="group flex overflow-hidden rounded-2xl border border-jade/10 bg-white shadow-editorial-card lg:flex-col"><div className="relative min-h-[230px] w-[42%] shrink-0 overflow-hidden lg:h-56 lg:w-full"><Image src={highlight.image} alt={highlight.imageAlt} fill sizes="(max-width: 1024px) 42vw, 33vw" className="object-cover transition duration-700 group-hover:scale-[1.035]" /><div className="absolute inset-0 bg-gradient-to-t from-jade-dark/65 via-transparent to-transparent" /><span className="absolute bottom-4 left-4 text-[9px] font-extrabold uppercase tracking-[0.18em] text-saffron-light">{highlight.eyebrow}</span></div><div className="flex flex-1 flex-col p-5 sm:p-6"><h3 className="font-display text-[1.7rem] font-semibold leading-none text-jade">{highlight.title}</h3><p className="mt-4 text-xs leading-5 text-charcoal/64">{highlight.description}</p><p className="mt-4 border-t border-jade/10 pt-4 text-[10px] font-semibold leading-4 text-jade/70">{highlight.decision}</p><span className="mt-auto inline-flex items-center justify-end gap-2 pt-5 text-xs font-extrabold text-jade">Bekijk de keuze <ArrowRight size={14} className="text-saffron transition group-hover:translate-x-1" /></span></div></Link>)}
            </div>
