...
function createSchemas(data: PatongExperienceData) {
  ...
  return [
      ...
      itemListElement: [
        {
          ...
          position: 1,
          name: "Thailand",
          item: `https://go2-thailand.com${prefix}/`,
        },
        {
          "@type": "ListItem",
          position: 2,
          name: "Phuket",
          item: `https://go2-thailand.com${prefix}/city/phuket/`,
        },
        {
          "@type": "ListItem",
          position: 3,
          name: "Patong",
          item: `https://go2-thailand.com${prefix}/phuket/patong/`,
        },
        {
          "@type": "ListItem",
          position: 4,
    ...
    },
    {
      "@context": "https://schema.org",
      "@type": "ItemList",
      name: `${data.breadcrumbLabel} planning zones`,
...
export function PatongExperienceTemplate({
  data,
}: {
  data: PatongExperienceData;
}) {
  ...
  return (
    <div
      className="bg-canvas"
      data-premium-template={`patong-experience-${isNl ? "nl" : "en"}`}
    >
      <SEOHead
        title={data.title}
        description={data.description}
      ...
      <EditorialHero
        ...
        breadcrumbs={[
          ...
          { label: "Patong", href: "/phuket/patong/" },
        ...
        disclosure={
          isNl
            ? "De providerlink is gesponsord. Wij kunnen zonder extra kosten voor jou een commissie ontvangen. Controleer aanbod, inhoud, beschikbaarheid, totaal en annulering."
            : "The provider link is sponsored. We may earn a commission at no extra cost to you. Check the current listing, inclusions, availability, total and cancellation terms."
        }
        minHeightClassName="min-h-[750px] lg:min-h-[700px]"
        titleClassName="max-w-[740px] text-[4rem] leading-[0.84] sm:text-[5rem] lg:text-[5.8rem]"
        contentClassName="max-w-[720px]"
        imageClassName="object-cover object-[68%_center] lg:object-center"
        gradientClassName="bg-[linear-gradient(180deg,rgba(248,244,235,0.08)_0%,rgba(248,244,235,0.48)_47%,rgba(248,244,235,0.99)_100%)] lg:bg-[linear-gradient(90deg,rgba(248,244,235,0.98)_0%,rgba(248,244,235,0.89)_38%,rgba(9,44,38,0.22)_66%,rgba(9,44,38,0.02)_100%)]"
      />
      <PageSectionNav
        label={
          isNl
            ? `In deze Patong-gids over ${data.breadcrumbLabel.toLowerCase()}`
            : `On this Patong ${data.breadcrumbLabel.toLowerCase()} guide`
        }
        items={data.navItems}
      />
        ...
        <div className="container-custom grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-start">
          ...
          <div>
            <div className="grid gap-px overflow-hidden rounded-2xl border border-jade/10 bg-jade/10 sm:grid-cols-2">
              {data.introCards.map(
                ({ eyebrow, title, copy, icon: Icon, tone }, index) => {
                  const dark =
                    tone === "dark" || index === data.introCards.length - 1;
                  return (
                    <article
                      key={title}
            ...
            <p className="mt-5 rounded-xl border border-saffron/25 bg-saffron-pale px-5 py-4 text-xs font-extrabold leading-6 text-jade">
              {isNl ? "Redactionele grens" : "Editorial rule"}: {data.editorialRule}
            </p>
          </div>
        </div>
      </section>
                ...
                <div className="flex min-h-[230px] flex-col p-6">
                  ...
                  <div className="mt-auto rounded-xl bg-tonal p-4">
                    <p className="text-[9px] font-extrabold uppercase tracking-[0.14em] text-saffron-dark">
                      {isNl ? "Actuele controle" : "Live check"}
                    </p>
      ...
      >
        <div className="container-custom grid gap-10 lg:grid-cols-[.72fr_1.28fr]">
          <SectionHeading
            eyebrow={data.choiceEyebrow}
            title={data.choiceTitle}
            description={data.choiceDescription}
          />
          <div className="grid gap-4 sm:grid-cols-2">
            {data.choices.map(({ title, copy, check, icon: Icon }) => (
              <article
                key={title}
              ...
              >
                ...
                <p className="mt-5 border-t border-jade/10 pt-4 text-[10px] font-extrabold leading-5 text-saffron-dark">
                  {isNl ? "Controleer" : "Check"}: {check}
                </p>
              </article>
            ))}
          </div>
      ...
      >
        <div className="container-custom">
          <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end">
            <SectionHeading
              eyebrow={data.rhythmEyebrow}
              title={data.rhythmTitle}
              description={data.rhythmDescription}
            />
            <p className="text-xs font-medium leading-6 text-charcoal/58">
              {isNl
                ? "Dit zijn beslisvensters, geen garantie op opening of drukte. Controleer locatie, vervoer en voorwaarden voor de echte avond."
                : "These are decision bands, not opening-hour or crowd guarantees. Verify the venue, transport and conditions for the actual evening."}
            </p>
          </div>
          <div className="mt-10 overflow-hidden rounded-2xl border border-jade/10 bg-jade/10">
            <div className="hidden grid-cols-[.55fr_1.2fr_1.2fr_.7fr] bg-jade px-6 py-4 text-[9px] font-extrabold uppercase tracking-[0.14em] text-white md:grid">
              <span>Moment</span>
              <span>{isNl ? "Typische sfeer" : "Typical feel"}</span>
              <span>{isNl ? "Plan rond" : "Plan around"}</span>
              <span>{isNl ? "Signaal" : "Cue"}</span>
            </div>
            {data.rhythmRows.map((row) => (
              <article
                key={row.period}
      ...
      >
        <div className="container-custom grid gap-10 lg:grid-cols-[.72fr_1.28fr]">
          <div>
            <p className="eyebrow !text-saffron-light">{data.featureEyebrow}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {data.featureCards.map(({ title, copy, icon: Icon }) => (
              <article
                key={title}
      ...
      >
        <div className="container-custom">
          ...
          <AffiliateDisclosure className="mt-4 !text-white/55">
            {isNl
              ? "Actuele provideropties zijn gesponsord. Wij kunnen een commissie ontvangen zonder jouw prijs te verhogen; interne redactionele links verdienen niets."
              : "Live provider options are sponsored. We may earn a commission without increasing your price; internal editorial links do not earn one."}
          </AffiliateDisclosure>
        </div>
      </section>
      ...
      <FaqSplitSection
        ...
        title={isNl ? `Vragen over ${data.breadcrumbLabel.toLowerCase()} in Patong` : `Questions about Patong ${data.breadcrumbLabel.toLowerCase()}`}
        description={data.faqDescription}
        items={data.faqs}
      />
      <RelatedGuidesSection
        eyebrow={isNl ? "Plan verder" : "Continue planning"}
        title={isNl ? "Bouw de rest van je verblijf in Patong" : "Build the rest of your Patong stay"}
