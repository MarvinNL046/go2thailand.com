      ...
      <EditorialHero
        ...
        sideCard={<aside className="absolute bottom-8 right-[max(1.25rem,calc((100vw-1200px)/2))] hidden w-[290px] rounded-2xl border border-white/45 bg-white/88 p-5 text-jade shadow-[0_18px_50px_rgba(18,63,54,0.18)] backdrop-blur-md lg:block">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 border-t border-jade/10 pt-4">
            {[[guide.duration, 'duur'], [guide.network, 'kust'], [guide.moves, 'tempo']].map(([value, label]) => <div key={label}><p className="text-[10px] font-extrabold uppercase tracking-[.12em] text-charcoal/45">{label}</p><p className="mt-1 text-xs font-extrabold leading-4 text-jade">{value}</p></div>)}
          </div>
      ...
      <section id="flexibel" className="section-divider-bottom scroll-mt-24 py-16 lg:py-24">
        <div className="container-custom">
          ...
          <div className="mt-12 overflow-hidden rounded-2xl bg-jade text-white lg:grid lg:grid-cols-[.72fr_1.28fr]">
            ...
            <div className="grid gap-px bg-white/10 sm:grid-cols-3">{guide.handoffs.map((handoff, index) => <div key={handoff.title} className="bg-jade p-7"><Route size={21} className="text-saffron" /><p className="mt-5 text-[10px] font-extrabold uppercase tracking-[.13em] text-white/45">Overgang {index + 1}</p><h3 className="mt-2 font-display text-[1.45rem] font-semibold leading-none">{handoff.title}</h3><p className="mt-4 text-sm font-medium leading-6 text-white/62">{handoff.copy}</p></div>)}</div>
          </div>
        </div>
      </section>
