...
class OpenAIClient {
  constructor() {
    ...
    this.rateLimiter = new RateLimiter(20); // Increased to 20 requests per minute for translations
  }

  // Helper: OpenAI API call with rate limiting and robust error handling
  async makeOpenAICall(prompt, maxTokens = 3500) {
    ...
    try {
      ...
      try {
        ...
        console.error('Received content:', content.substring(0, 200) + '...');
        return {
          error: true,
...
- enhanced_description: 800-1200 word immersive story-style description that:
  * Unique opening specific to ${cityData.name.en}
  * Do NOT use "Picture this" or generic clichés
  * Start with city-specific sensory details (Bangkok: traffic/markets, Chiang Mai: mountains/temples, Phuket: beaches/boats, Krabi: limestone cliffs)
  * Include personal anecdotes that could ONLY happen in ${cityData.name.en}
  * Explain WHY this city is special vs other Thailand destinations
  * Use storytelling with specific local details, not generic facts
  * Make readers feel the unique atmosphere of ${cityData.name.en}
...
{
  "enhanced_description": "800-1200 word immersive story with unique opening - NO 'Picture this' clichés",
  "hidden_gems": [
    {
      "name": "Specific place name",
      "story": "Personal insider story",
      "how_to_find": "Detailed directions",
      "best_time": "When to visit",
      "local_insights": ["Array of insider tips"]
    }
  ],
  "authentic_experiences": [
    {
      "activity": "Specific local activity",
      "story": "Personal experience story", 
      "cultural_significance": "Why this matters to locals",
      "how_to_participate": "Practical steps",
      "insights": ["Cultural tips"]
    }
  ],
  "foodie_adventures": [
    {
      ...
      "story": "Personal taste story",
      "price_range": "Actual baht amounts",
      "ordering_tips": ["How to order like local"]
    }
  ],
  "local_insights": [
    {
      "observation": "What surprised you about local life",
      "tip": "Practical advice for visitors",
      "surprise": "Personal discovery"
    }
  ],
  "seasonal_secrets": {
    "best_times": {"month": "why this timing is special"},
    "local_festivals": ["Festival names and dates"],
    "seasonal_foods": {"season": "special foods available"},
    "insider_tips": {"season": "local secrets"}
  },
  "budget_reality": {
    "daily_costs": {"meal_type": "Specific example: 40 baht for pad thai"},
    "money_saving_tricks": ["Specific tips locals use"],
    "where_to_splurge": ["Worth spending more on"],
    "hidden_costs": ["Unexpected expenses tourists face"]
  }
}`;
