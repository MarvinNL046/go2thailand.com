...
class PerplexityClient {
  ...
  getQueryTemplate(cityName, category) {
    const templates = {
      restaurants: `Top 10 best restaurants in ${cityName}, Thailand in 2026. Include for each: current prices in Thai Baht, opening hours, exact location/address, recent reviews or ratings, specialties/signature dishes, any new openings in 2024-2026. Focus on authentic local favorites and highly rated establishments.`,
      
      hotels: `Top 10 hotels in ${cityName}, Thailand in 2026. Include for each: current price ranges in Thai Baht, key amenities, exact location, recent guest reviews, booking availability trends, what makes each unique. Mix of budget, mid-range, and luxury options.`,
      
      attractions: `Top 10 attractions and activities in ${cityName}, Thailand in 2026. Include for each: current entrance fees in Thai Baht, opening hours, exact location, seasonal information, any new attractions opened in 2024-2026, visitor tips, what makes each special.`
    };
  ...
  createFallbackContent(query) {
    ...
    if (query.includes('restaurants')) {
      return `Top restaurants in ${city} (2026):
...
10. Err Urban Rustic Thai - Modern rustic, 800-1500 THB per person, 394/35 Maharaj Rd`;
    ...
    if (query.includes('hotels')) {
      return `Top hotels in ${city} (2026):
...
10. Praya Palazzo - Historic charm, 2800-5000 THB per night, 757/1 Somdej Phra Pinklao Rd`;
    ...
    if (query.includes('attractions')) {
      return `Top attractions in ${city} (2026):
