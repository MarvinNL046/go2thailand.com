...
function getRegionBySlug(slug, locale = 'en') {
  try {
    if (locale && locale !== 'en') {
      const localePath = path.join(process.cwd(), 'data', 'regions', locale, `${slug}.json`);
      if (fs.existsSync(localePath)) return JSON.parse(fs.readFileSync(localePath, 'utf8'));
    }
