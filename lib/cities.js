...
function generateCityMetadata(city, category = '', locale = 'en') {
  if (!city) return {};

  const contentLocale = locale === 'nl' ? 'nl' : 'en';

  // Use enhanced description if available
  const rawCityDesc = city.overview || city.enhanced_description || city.seo?.metaDescription?.[contentLocale] || city.description?.[contentLocale] || city.description?.en || '';
  const description = rawCityDesc.length > 155 ? rawCityDesc.substring(0, 155) + '...' : rawCityDesc;

  const baseTitle = city.seo?.metaTitle?.[contentLocale] || city.seo?.metaTitle?.en || `${city.name.en} Travel Guide: Things to Do, Where to Stay & Food`;
  const title = category ?
    `${city.categories?.[category]?.[contentLocale] || city.categories?.[category]?.en || baseTitle}` :
    baseTitle;

  const optimizedDescription = city.seo?.metaDescription?.[contentLocale] || city.seo?.metaDescription?.en ||
    (contentLocale === 'nl'
      ? `Plan je reis naar ${city.name.nl || city.name.en} met tips over bezienswaardigheden, verblijf, eten en vervoer.`
