...
function generateCityMetadata(city, category = '') {
  ...
  const baseTitle = city.seo?.metaTitle?.en || `Discover ${city.name.en} | Thailand Travel Guide`;
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: getCityImageForSection(city, 'hero'),
          width: 1200,
          height: 630,
...
function getAllAttractionStaticPaths() {
  ...
  // For now, only Bangkok has attractions data
