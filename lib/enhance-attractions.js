...
async function enhanceAttractionContent(attraction) {
  const prompt = `You are a travel expert specializing in Thailand tourism. Create detailed, engaging content for this Bangkok attraction:
...

{
  ...
  "practical_info": {
    "how_to_get_there": "Transportation options and directions",
...
Focus on accuracy, engaging storytelling, and practical value for tourists. Make it informative but not overwhelming.`;

  try {
    console.log(`🤖 Generating enhanced content for ${attraction.name.en}...`);
...
async function enhanceAllAttractions(citySlug) {
  ...
  console.log(`\n🚀 Starting enhancement for ${attractions.length} ${citySlug} attractions...\n`);
  ...
  console.log(`\n🎉 Enhancement completed for ${citySlug}!`);
}
...
if (require.main === module) {
  ...
  console.log(`🔄 Enhancing attractions for ${citySlug}...`);
  
  enhanceAllAttractions(citySlug)
    .then(() => {
      console.log(`\n✅ All ${citySlug} attractions enhanced successfully!`);
    })
    .catch(error => {
...
module.exports = { enhanceAttractionContent, enhanceAllAttractions };
