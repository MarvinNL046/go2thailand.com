...
function generateSitemap() {
    ...
    if (fs.existsSync(clustersDir)) {
      ...
      cityFolders.forEach(cityFolder => {
        ...
        if (fs.existsSync(hubFile)) {
          ...
          clusterPages.push(`/guides/${cityFolder}-travel-guide/`);
          clusterCount++;
        }
      });
  ...
  // Non-EN/NL locales only get: static/index pages + their own translated blog posts
  const nonEnStaticPages = [
    ...staticPages,
    ...top10CategoryPages,
  ];
  ...
  locales.forEach(locale => {
    ...
    if (locale === 'en') {
      ...
      // Other locales: only static/index pages + their translated blog posts
      nonEnStaticPages.forEach(page => {
...
function generateSitemapXML(urls, locale = 'en') {
  ...
  const urlElements = urls.map(url => {
    ...
    if (url === '/' || url === `/${locale}/`) {
      ...
      priority = '0.7';
      changefreq = 'weekly';
    } else if (url.startsWith('/guides/where-to-stay-') || url.startsWith('/guides/') && url.endsWith('-travel-guide/')) {
      // Cluster guide pages
      priority = '0.6';
      changefreq = 'weekly';
    } else {
