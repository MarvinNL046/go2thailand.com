...
const { getAllDivingSnorkelingCities } = require('./diving-snorkeling');
...
function getStaticPages() {
  const staticPages = [
    ...
    '/travel-insurance-thailand/', // Travel insurance page
...
function generateSitemap() {
  ...
  cities.forEach(city => {
    ...
    cityPages.push(`/city/${city.slug}/budget/`);
  });
  ...
  // Diving & snorkeling pages - per city
  const divingSnorkelingPages = [];
  const divingSnorkelingCities = getAllDivingSnorkelingCities();
  divingSnorkelingCities.forEach(citySlug => {
    divingSnorkelingPages.push(`/city/${citySlug}/diving-snorkeling/`);
  });
  ...
  // Non-EN/NL locales: build pages based on actual translated content
  function getTranslatedPagesForLocale(locale) {
    ...
    // Top 10 city pages — include if locale translation file exists
    top10Guides.forEach(guide => {
      const localePath = path.join(__dirname, '..', 'data', 'top10', locale, `${guide.city}-${guide.category}.json`);
      if (fs.existsSync(localePath)) {
        pages.push(guide.url);
      }
    });

    // City main pages — include if city JSON has name.[locale]
    cities.forEach(city => {
      try {
    ...
    // Region pages — include if region JSON has name.[locale]
    regions.forEach(region => {
      try {
        const regionFile = path.join(__dirname, '..', 'data', 'regions', `${region.slug}.json`);
        if (fs.existsSync(regionFile)) {
          const regionData = JSON.parse(fs.readFileSync(regionFile, 'utf8'));
          if (regionData.name && typeof regionData.name === 'object' && regionData.name[locale]) {
            pages.push(`/region/${region.slug}/`);
          }
        }
      } catch (e) {}
    });

    // Food pages — include if food JSON has translations.[locale] or name.[locale]
    dishes.forEach(dish => {
      try {
  ...
  locales.forEach(locale => {
    ...
    if (locale === 'en') {
      ...
      // NL gets all bilingual pages (no transport — EN-only, no EN blog — NL blog separate)
      bilingualPages.forEach(page => {
      ...
      // Other locales: pages based on actual translated content
      const translatedPages = getTranslatedPagesForLocale(locale);
      translatedPages.forEach(page => {
        const localePage = page === '/' ? `/${locale}/` : `/${locale}${page}`;
        localePages.push(localePage);
      });
      const localeBlogSlugs = blogPostsByLocale[locale] || [];
      localeBlogSlugs.forEach(slug => {
        localePages.push(`/${locale}/blog/${slug}/`);
      });
    }
...
function generateSitemapXML(urls, locale = 'en') {
  ...
  const urlElements = urls.map(url => {
    ...
    if (url === '/' || url === `/${locale}/`) {
      ...
      priority = '0.9';
      changefreq = 'weekly';
    } else if (url.includes('/cooking-classes/') || url.includes('/muay-thai/') || url.includes('/elephant-sanctuaries/') || url.includes('/diving-snorkeling/')) {
