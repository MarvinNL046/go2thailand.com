...
function getStaticPages() {
  ...
  // Add region pages if they exist
  const regionPages = ['/region/northern/', '/region/central/', '/region/southern/'];
  // For now, we'll add them assuming they exist or will exist
  staticPages.push(...regionPages);
...
function generateSitemap() {
  ...
  console.log(`📊 Data loaded: ${cities.length} cities, ${dishes.length} dishes, ${attractions.length} attractions, ${top10Guides.length} top-10 guides`);
...
function getSitemapStats() {
  ...
  return {
    ...
    totalEstimated: 3 + dishes.length + (cities.length * 4) + attractions.length + 3 // static + food + city pages + attractions + regions
  };
}
