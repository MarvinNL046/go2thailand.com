...
function generateSitemapXML(urls, locale = 'en') {
  ...
  const urlElements = urls.map(url => {
    ...
    if (url === '/' || url === `/${locale}/`) {
      ...
      priority = '0.8';
      changefreq = 'weekly';
    } else if (url.includes('/best-beaches-in-thailand') || url.includes('/best-cooking-classes-in-thailand')) {
      // Best-of guide pages
      priority = '0.9';
      changefreq = 'weekly';
    } else if (url.includes('/cooking-classes/')) {
      // City cooking class pages
      priority = '0.8';
      changefreq = 'weekly';
