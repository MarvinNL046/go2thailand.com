...
function generateSitemap() {
  ...
  locales.forEach(locale => {
    ...
    if (locale === 'en') {
      ...
      // NL: high-value pages only — static pages, city index, food, drinks, regions, islands, top10, itineraries, clusters
      const nlPages = [
        ...staticPages,
        ...foodPages.map(p => p),
        ...drinkPages.map(p => p),
        ...regionPages.map(p => p),
        ...islandPages.map(p => p),
        ...itineraryPages.map(p => p),
        ...clusterPages.map(p => p),
      ];
      // City main pages only (no subpages like /food/, /hotels/, /attractions/, /weather/)
      cities.forEach(city => {
        nlPages.push(`/city/${city.slug}/`);
      });
      // Top 10 pages
      top10Pages.forEach(page => {
        nlPages.push(page);
      });
      nlPages.forEach(page => {
