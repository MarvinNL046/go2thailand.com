...
const BLOG_CONSOLIDATIONS = {
  en: {
    ...
    'best-snorkeling-spots-thailand-water-clarity': '/best-diving-snorkeling-in-thailand/',
    'bangkok-street-food-beginners': '/blog/best-street-food-markets-bangkok/',
    'cheapest-vs-most-expensive-hotel-bangkok': '/best-hotels/bangkok/',
    'how-much-2-weeks-thailand-cost-2026-budget': '/thailand-index/budget/',
    'is-thailand-safe-tourists-2026': '/is-thailand-safe/',
    'kpop-concerts-bangkok-2026-seventeen-treasure-bts-guide': '/blog/bts-world-tour-bangkok-december-2026-tickets-guide/',
    'mango-sticky-rice-season-thailand': '/food/mango-sticky-rice/',
    'pad-thai-street-food-vs-restaurant-homemade': '/food/pad-thai/',
    'som-tam-regional-variations-thailand': '/food/som-tam/',
    'tom-yum-goong-guide': '/food/tom-yum-goong/',
    'what-is-thai-food-cuisine-guide': '/travel-guides/thai-cuisine-food-guide/',
    'thai-street-food-guide-2026': '/thailand-street-food/',
    'thailand-health-vaccinations': '/blog/thailand-health-vaccinations-doctors-recommend/',
    'thailand-king-cobra-season': '/blog/thailand-king-cobra-season-travelers-guide/',
    'solo-female-travel-thailand-safety-tips': '/travel-guides/solo-female-travel-thailand/',
    'thailand-budget-2026-daily-costs': '/thailand-index/budget/',
    'thailand-budget-backpacker-guide': '/thailand-index/budget/',
    'thailand-budget-vs-comfort-travel': '/thailand-index/budget/',
    'thailand-cheap-2026-travel-costs': '/thailand-index/budget/',
    'thailand-digital-arrival-card-tdac-guide': '/visa/digital-arrival-card/',
    'thailand-travel-scams-2026': '/practical-info/scams-safety/',
    'thailand-visa-changes-2026': '/visa/',
    'thailand-visa-free-stay-cut-60-to-30-days-2026': '/visa/visa-free-entry/',
    'thailand-visa-guide-2026': '/visa/',
    'where-to-stay-chiang-mai-neighborhoods': '/where-to-stay/chiang-mai/',
  },
  nl: {
    ...
    'phuket-weer': '/nl/city/phuket/weather/',
    'snorkelen-in-thailand-beste-eilanden-voor-heldere-zicht': '/nl/travel-guides/diving-snorkeling-thailand/',
    'snorkelen-thailand-beste-eilanden-heldere-zicht': '/nl/travel-guides/diving-snorkeling-thailand/',
    'thailand-mekong-riverside-seven-provinces-sustainable-tourism-2026': '/nl/itineraries/mekong-river-adventure/',
    'thailand-rainy-season-travel-tips-2026': '/nl/weather/',
    'thailand-rising-costs-tourists-middle-east-crisis-2026': '/nl/thailand-index/budget/',
    'how-much-2-weeks-thailand-cost-2026-budget': '/nl/thailand-index/budget/',
    ...
    'thailand-budget-backpacker-guide': '/nl/thailand-index/budget/',
    ...
    'thai-street-food-guide-2026': '/nl/food/',
  },
};
...
function canonicalBlogPath(pathname, locale) {
  ...
  const exact = new Map([
    ...
    ['/blog/pad-thai-streetfood-vs-restaurant-thuisgemaakt/', '/food/pad-thai/'],
    ['/blog/som-tam-regionale-variaties-thailand/', '/food/som-tam/'],
    ...
    ['/blog/solo-female-travel-thailand-safety-tips/', '/travel-guides/solo-female-travel-thailand/'],
...
// The modern article owner already renders contextual, disclosed CTA blocks.
// Historical generated widgets are duplicated commercial UI and contain
// deeply nested inline styling, so remove each balanced widget block before
// Markdown rendering while preserving ordinary editorial HTML.
function stripLegacyWidgetBlocks(markdown) {
...
function getEditorialVolatility(content) {
  const hasPriceOrSchedule = /(?:\bTHB\b|\bbaht\b|\bUSD\b|[$ۣ]\s?\d|\d[.,]?\d*\s?(?:THB|baht|euros?|EUR|USD)|\bopening hours?|\bopen daily|\bdeparts every|\bevery \d+ minutes?|\b\d{1,2}:\d{2}\s?(?:am|pm)?)/i.test(content);
  const hasFirstPersonAuthorityClaim = /\b(?:we|our team|the Go2Thailand team|I)\s+(?:visited|stayed|lived|have lived|have spent years|have seen|found|watched|experienced)|\b(?:our years|my years|firsthand experience|during our three years|countless trips)\b/i.test(content);
  return { hasPriceOrSchedule, hasFirstPersonAuthorityClaim };
}
...
function getPostsForIndex(locale = 'en') {
  ...
  const archivedNlSlugs = new Set();
  if (locale === 'nl') {
    const profileDirectory = path.join(process.cwd(), 'data', 'editorial', 'blog', 'nl');
    if (fs.existsSync(profileDirectory)) {
      for (const file of fs.readdirSync(profileDirectory).filter(name => name.endsWith('.json'))) {
        ...
        if (profile.editorialStatus === 'archived' || profile.seo?.noindex === true) {
          archivedNlSlugs.add(file.replace(/\.json$/, ''));
        }
      }
    }
  }
  return getAllPosts(locale).filter(p => !getBlogConsolidation(locale, p.slug) && !archivedNlSlugs.has(p.slug)).map(p => ({
    slug: p.slug,
    title: p.title || '',
...
async function getPostBySlug(slug, locale = 'en') {
  ...
  const safeContent = stripLegacyWidgetBlocks(content);
  const editorialVolatility = getEditorialVolatility(content);
  ...
  return {
    ...
    author: normalizeAuthor(data.author),
    tags: Array.isArray(data.tags) ? data.tags.filter(Boolean).map(String) : [],
    readingTime: Number(data.readingTime) || Math.max(1, Math.ceil(safeContent.split(/\s+/).length / 220)),
    image: resolveBlogImage(data.image, slug),
    date: data.date ? String(data.date) : '',
  };
}
...
function getPostsByCategory(category, locale = 'en') {
  const posts = getAllPosts(locale);
  return posts.filter(post => post.category === category && !getBlogConsolidation(locale, post.slug));
}
...
function getPostsByTag(tag, locale = 'en') {
  ...
  return posts.filter(post =>
    !getBlogConsolidation(locale, post.slug) && post.tags && Array.isArray(post.tags) && post.tags.some(t => slugifyTag(t) === tag)
  );
}
...
function getFeaturedPosts(locale = 'en') {
  const posts = getAllPosts(locale);
  return posts.filter(post => post.featured && !getBlogConsolidation(locale, post.slug));
}
...
function getRelatedPosts(currentSlug, locale = 'en', limit = 3) {
  ...
  if (!currentPost) return posts.filter(post => !getBlogConsolidation(locale, post.slug)).slice(0, limit);

  const currentTags = (currentPost.tags || []).map(t => slugifyTag(t));
  const others = posts.filter(post => post.slug !== currentSlug && !getBlogConsolidation(locale, post.slug));
...
function getPostStaticPaths(locale = 'en') {
  const posts = getAllPosts(locale).filter(post => !getBlogConsolidation(locale, post.slug));
