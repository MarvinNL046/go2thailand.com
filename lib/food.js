...
// Get enhanced dish by slug. Prefers locale-specific enhanced data
// (e.g. data/enhanced/food/nl/<slug>.json), falls back to English enhanced,
// then to the base dish file.
function getEnhancedDishBySlug(slug, locale = 'en') {
  try {
    if (locale && locale !== 'en') {
      try { return require(`../data/enhanced/food/${locale}/${slug}.json`); } catch (_) {}
    }
    try {
...
function generateDishMetadata(dish) {
  ...
  const title = `${dish.name.en}: Recipe, Best Spots & Street Food Tips (2026)`;
  const description = `🍜 ${dish.name.en} (${dish.name.thai}) - ${(dish.enhanced_description || dish.description.en).substring(0, 120)}. Find recipes, best restaurants & prices!`;
  const keywords = [
    dish.name.en,
    dish.name.thai,
    'Thai food',
