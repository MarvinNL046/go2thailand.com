...
const configs: Config[] = [
  ...
  },
  {
    ...
    affiliate: 'klook', minSponsored: 6,
    requiredLinks: ['/nl/things-to-do-in-thailand/', '/nl/city/', '/nl/weather/'],
    markers: ['Niet elke top-tour is jouw beste dag.', 'Vier ervaringen die Thailand anders laten voelen.', 'Welke excursie past in je route?', 'Veelgestelde vragen over excursies in Thailand'],
  },
  {
    route: '/nl/transport/', titleNeedle: 'vervoer in thailand', minSections: 11, minWords: 1250, minImages: 7,
    affiliate: '12go', minSponsored: 3,
    requiredLinks: ['/nl/itineraries/', '/nl/city/', '/nl/practical-info/'],
    markers: ['Van waar naar waar?', 'Zes manieren. Zes andere afwegingen.', 'Begin bij de route, niet bij het voertuig.', 'Veelgestelde vragen over vervoer in Thailand'],
  },
];
...
async function inspect(config: Config, row: KeywordRow): Promise<Result> {
  ...
  for (const required of ['Organization', 'CollectionPage', 'ItemList', 'FAQPage', 'BreadcrumbList']) if (!schemaTypes.has(required)) errors.push(`${required}-schema ontbreekt`);
  ...
  if (config.affiliate === 'none' && sponsored.length) errors.push('informatieve bestemmingshub bevat onverwachte affiliate-uitgangen');
  for (const tag of sponsored) {
    const href = decodeHtml(attr(tag, 'href') || '');
    const rel = attr(tag, 'rel')?.split(/\s+/) || [];
    if (config.affiliate === 'klook' && !/klook\.tpo\.lv|klook\.com/.test(href)) errors.push('affiliate-uitgang is niet van Klook');
    if (config.affiliate === '12go' && !/12go\.tpo\.lv|12go\.asia/.test(href)) errors.push('affiliate-uitgang is niet van 12Go');
  ...
  if (config.affiliate !== 'none' && !text.toLowerCase().includes('affiliate')) errors.push('affiliate-uitleg ontbreekt');
  if (anchors.some((tag) => /\/go\//.test(attr(tag, 'href') || ''))) errors.push('hubintentie is verwaterd met Amazon-productlinks');
  ...
  if (sourceLinks.length < 2) errors.push(`slechts ${sourceLinks.length} redactionele bronlinks`);
...
async function main(): Promise<void> {
  ...
  console.log(`NL core-hub verification: ${configs.length - failures.length}/${configs.length} owners groen op ${baseUrl}.`);
  console.log('DFS/PAA, metadata, landmarks, CollectionPage-schema, affiliates, bronnen, visuals en alle interne links gecontroleerd.');
