...
import FlightOriginGuideNl from "../../components/flights/FlightOriginGuideNl";
...
export default function FlightFromOriginPage({
  route,
  siblings,
  generic,
  lastUpdated,
  ...
  const seoDescription = isNl
    ? `Vluchten van ${route.fromName} naar Phuket: ${route.duration}, ${route.stops}, ${route.priceBand}. Vergelijk airlines en boek via Trip.com.`.slice(
        0,
        160,
      )
    : `Flights from ${route.fromName} to Phuket: ${route.duration}, ${route.stops}, typical fares ${route.priceBand}. Compare airlines and book via Trip.com.`.slice(
        0,
        160,
      );
        ...
        <section className="bg-thailand-blue text-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="mt-4 flex items-center gap-3">
              <span className="rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-semibold uppercase tracking-wide">
                {tierBadge(route.tier, isNl)}
              </span>
            ...
            <p className="mt-4 text-lg max-w-3xl opacity-95">
              {route.duration} · {route.stops} · {route.frequency} ·{" "}
              {route.priceBand}
            </p>
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              >
                {isNl
                  ? `Zoek vluchten ${route.fromName} → Phuket`
                  : `Find flights ${route.fromName} → Phuket`}{" "}
                →
              </a>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          ...
          <section className="rounded-2xl bg-white p-6 shadow-sm border border-gray-200">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {isNl ? "Route in cijfers" : "Route at a glance"}
            </h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-500 font-semibold">
                  {isNl ? "Vluchttijd" : "Flight time"}
                </p>
              ...
              <div>
                ...
                <p className="mt-1 font-heading font-semibold text-gray-900">
                  {route.frequency}
                </p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-gray-500 font-semibold">
                  {isNl ? "Prijsklasse" : "Typical price"}
                </p>
          ...
          <section>
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-3">
              {isNl ? `Airlines op deze route` : `Airlines on this route`}
            </h2>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {route.airlines.map((a) => (
                <span
                  key={a}
                  className="rounded-full bg-thailand-blue/10 text-thailand-blue px-3 py-1 text-sm font-semibold"
                >
                  {a}
                </span>
              ))}
            ...
            >
              {isNl
                ? "Bekijk live tarieven op Trip.com"
                : "See live fares on Trip.com"}{" "}
              →
            </a>
          </section>
          ...
          {siblings.length > 0 && (
            <section>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
                {isNl
                  ? `Andere routes naar Phuket ${route.tier === "domestic" ? "binnen Thailand" : route.tier === "regional" ? "vanuit Azië" : ""}`.trim()
                  : `Other ${route.tier === "domestic" ? "domestic" : route.tier === "regional" ? "regional" : "long-haul"} routes to Phuket`}
              </h2>
                ...
                {siblings.slice(0, 6).map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="font-heading font-bold text-gray-900 mt-1">
                        {s.fromName}
                      </p>
          ...
          <section className="rounded-2xl bg-thailand-blue/10 p-6">
            <h2 className="font-heading text-xl font-bold text-gray-900">
              {isNl
                ? `Eenmaal in Phuket aangekomen`
                : `Once you land in Phuket`}
            </h2>
            <p className="mt-2 text-gray-700">
              {isNl
                ? "Plan de rest van je Phuket trip:"
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                {isNl
                  ? "Alle 15 routes naar Phuket"
                  : "All 15 routes to Phuket"}
              </Link>
              <Link
...
export const getStaticProps: GetStaticProps<Props> = async ({
  ...
  const siblings = routes.filter(
    (r) => r.tier === route.tier && r.code !== code && r.tier !== "search",
  );
