...
import PhuketFlightsGuideNl from "../../components/flights/PhuketFlightsGuideNl";
...
const tierLabel = (tier: string, isNl: boolean): string => {
  if (isNl) {
    switch (tier) {
      case "domestic":
        return "Vanuit Thailand";
      case "regional":
        return "Vanuit Azië";
      case "long-haul":
        return "Intercontinentaal (1+ tussenstops)";
      default:
        return "";
    }
  }
...
export default function FlightsToPhuketPage({ routes, lastUpdated }: Props) {
  ...
  if (isNl) {
    return <PhuketFlightsGuideNl routes={routes} />;
  }

  const tiers: Array<"domestic" | "regional" | "long-haul"> = [
    "domestic",
    "regional",
    "long-haul",
  ];
  ...
  const breadcrumbs = [
    ...
    {
      name: isNl ? "Vluchten naar Phuket" : "Flights to Phuket",
  ...
  const itemListJsonLd = {
    ...
    "@type": "ItemList",
    name: isNl
      ? "Routes naar Phuket International Airport"
      : "Routes to Phuket International Airport",
    itemListElement: routes
      .filter((r) => r.tier !== "search")
      .map((r, i) => ({
        "@type": "ListItem",
        position: i + 1,
  ...
  const faqEn = [
    ...
    },
    {
      ...
      answer:
        "Direct flights from Bangkok (BKK or DMK) take 1h 30m. There are 30+ daily flights from BKK and 20+ from DMK across Thai Airways, Bangkok Airways, AirAsia, Thai VietJet, Nok Air and Thai Lion Air. Total journey time including airport check-in and transfer to your Phuket hotel is typically 4–5 hours.",
    },
    {
      name: "Can you fly directly from the US to Phuket?",
      answer:
        "No — there are no non-stop flights from any US city to Phuket. All routes have at least one stop, typically in Bangkok (BKK), Hong Kong (HKG), Singapore (SIN), Taipei (TPE), Doha (DOH), or Dubai (DXB). Most LAX/SFO travelers connect via Taipei (EVA Air) or Hong Kong; JFK travelers usually connect via Doha (Qatar) or Dubai (Emirates).",
    },
    {
      name: "Is Phuket Airport (HKT) close to Patong?",
      answer:
        "No — Phuket International Airport sits at the north end of the island; Patong is a 45–60 minute drive south. Karon and Kata are 60–75 minutes. Mai Khao Beach hotels are the closest to the airport (10–20 minutes). Budget for a 600–1,200 THB metered taxi or pre-book an airport transfer through your hotel for fixed pricing.",
    },
    {
  ...
  return (
    <>
      <SEOHead title={seoTitle} description={seoDescription}>
        <link
          rel="canonical"
          href={`https://go2-thailand.com${isNl ? "/nl" : ""}/flights-to-phuket/`}
        />
        <script
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-1 text-xs opacity-90">
              ...
              <span>
                ✔{" "}
                {isNl
                  ? "Direct + 1 tussenstop opties"
                  : "Direct + 1-stop options"}
              </span>
              <span>
                ✔ {isNl ? "30-dagen cookie tracking" : "30-day cookie tracking"}
              </span>
              <span>
                ✔ {isNl ? "Eerlijke airline-notities" : "Honest airline notes"}
              </span>
            </div>
          </div>
        </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          ...
          <section id="comparison">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-2">
              {isNl
                ? "Snelle vergelijking: 15 routes naar Phuket"
                : "Quick comparison: 15 routes to Phuket"}
            </h2>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-gray-700">
                  <tr>
                    ...
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Reistijd" : "Duration"}
                    </th>
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Tussenstops" : "Stops"}
                    </th>
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Prijsklasse" : "Price band"}
                    </th>
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Zoek vluchten" : "Find flights"}
                    </th>
                  </tr>
                </thead>
                  ...
                  {tierRoutes.map((r) => (
                    ...
                    >
                      <div className="flex flex-wrap items-baseline gap-3 mb-2">
                        <h3 className="font-heading text-xl font-bold text-gray-900">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer nofollow sponsored"
                            className="hover:text-thailand-red"
                          >
                            {r.fromName} → Phuket
                          </a>
                        </h3>
                        <span className="rounded-full bg-thailand-blue/10 text-thailand-blue px-3 py-1 text-xs font-semibold uppercase tracking-wide">
                          {r.from} → HKT
                        </span>
                      </div>
                      <div className="grid sm:grid-cols-3 gap-4 text-sm mb-3">
                        ...
                        <div>
                          <p className="text-xs uppercase text-gray-500 font-semibold">
                            {isNl ? "Typische prijs" : "Typical price"}
                          </p>
          ...
          <section className="rounded-2xl bg-amber-50 border border-amber-200 p-6">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {isNl
                ? "Wanneer boeken: vluchten naar Phuket"
            ...
            {isNl ? (
              <ul className="space-y-3 text-gray-800">
                <li className="flex gap-3">
                  ...
                  <span>
                    <strong>Binnenland (BKK/CNX/USM):</strong> 4–6 weken vooruit
                    is het beste moment. Last-minute (binnen 7 dagen) zijn de
                    prijzen op Thai VietJet soms gelijk, maar bijna nooit lager.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Regionaal (Singapore/KL/HKG):</strong> 6–8 weken
                    vooruit. Vertrekken op dinsdag en woensdag zijn meestal
                    20–30% goedkoper dan in het weekend.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Intercontinentaal (LAX/JFK/LHR/AMS):</strong> 3–4
                    maanden vooruit voor het hoogseizoen (nov–apr); 1–2 maanden
                    voor het laagseizoen. Kijk specifiek naar vertrekken op
                ...
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Goedkoopste maanden:</strong> half september tot
                    begin november (net voor het hoogseizoen) en mei–juni (na
                    Songkran, voor de Europese zomervakantie).
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Vermijden:</strong> Kerst/Oud & Nieuw (tarieven 2–3x
                    normaal), Chinees Nieuwjaar (jan/feb) en Songkran (half
                    april). Boek 4–6 maanden vooruit als je in deze periodes
                    moet reizen.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Bagage-valkuil:</strong> Thai VietJet, AirAsia en
                    Scoot tonen lage tarieven zonder ruimbagage. Reken op
                    500–1.200 THB extra voor 20 kg ruimbagage. De totaalprijs
                    komt vaak in de buurt van een full-service airline.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Toeristenbelasting (2026):</strong> Thailand wil de
                    aankomstheffing van 300 THB versneld invoeren onder de
                    nieuwe regering. Controleer bij het boeken of deze al actief
                    is en door de airline bij ticketverkoop wordt geïnd.
                  </span>
                </li>
              </ul>
            ) : (
              <ul className="space-y-3 text-gray-800">
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Domestic (BKK/CNX/USM):</strong> 4–6 weeks ahead is
                    the sweet spot. Last-minute fares (within 7 days) sometimes
                    match advance prices on Thai VietJet but rarely beat them.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Regional (Singapore/KL/HKG):</strong> 6–8 weeks
                    ahead. Tuesday and Wednesday departures usually 20–30%
                ...
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Long-haul (LAX/JFK/LHR/AMS):</strong> 3–4 months
                    ahead for high season (Nov–Apr); 1–2 months for low season.
                    Check Tuesday-Wednesday-Saturday departures specifically.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Best months for cheapest fares:</strong>{" "}
                    mid-September through early November (just before high
                    season) and again May–June (post Songkran, pre-summer
                    European travel).
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Avoid:</strong> Christmas/New Year week (rates 2–3x
                    normal), Chinese New Year (Jan/Feb), and Songkran
                    (mid-April). Book 4–6 months out for these dates if you must
                    travel.
                  </span>
                </li>
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Baggage trap:</strong> Thai VietJet, AirAsia and
                    Scoot all show low fares without checked bags. Add 500–1,200
                    THB for 20kg checked. The full price may end up similar to a
                ...
                <li className="flex gap-3">
                  <span className="text-amber-700 font-bold">→</span>
                  <span>
                    <strong>Tourist tax (2026):</strong> Thailand&apos;s 300 THB
                    air-arrival entry fee is being fast-tracked under the new
                    administration. Confirm at time of booking whether it&apos;s
                    active and collected by airline at ticket purchase.
                  </span>
                </li>
              </ul>
            )}
          ...
          <section>
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {isNl
                ? "Phuket Airport (HKT) — praktische info"
                : "Phuket Airport (HKT) basics"}
            </h2>
          ...
          <section className="rounded-2xl bg-thailand-blue/10 p-6">
            <h2 className="font-heading text-xl font-bold text-gray-900">
              {isNl
                ? "Vlucht geboekt? Plan de rest van je reis"
                : "Once you’ve booked your flight"}
            </h2>
            <p className="mt-2 text-gray-700">
              {isNl
                ? "Plan de rest van je trip naar Phuket:"
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                🗺️{" "}
                {isNl ? "Waar overnachten (wijken)" : "Where to stay (areas)"}
              </Link>
              <Link
                href="/best-hotels/phuket/family/"
                className="rounded-full bg-white text-thailand-blue border border-thailand-blue px-5 py-2 text-sm font-semibold hover:bg-thailand-blue hover:text-white"
              >
                👨‍👩‍👧 {isNl ? "Familiehotels" : "Family hotels"}
              </Link>
              <Link
                href="/best-hotels/phuket/all-inclusive/"
                className="rounded-full bg-white text-thailand-blue border border-thailand-blue px-5 py-2 text-sm font-semibold hover:bg-thailand-blue hover:text-white"
              >
              ...
              <a
                href={withSubId(KLOOK_GENERIC, subId)}
                target="_blank"
                rel="noopener noreferrer nofollow sponsored"
              ...
              <a
                href={withSubId(TWELVEGO_GENERIC, subId)}
                target="_blank"
                rel="noopener noreferrer nofollow sponsored"
                className="rounded-full bg-white text-gray-900 border border-gray-300 px-5 py-2 text-sm font-semibold hover:bg-gray-50"
              >
...
export const getStaticProps: GetStaticProps<Props> = async ({ locale }) => {
  ...
  return {
    props: { routes: data.routes, lastUpdated: data.lastUpdated },
    revalidate: 604800,
  };
};
