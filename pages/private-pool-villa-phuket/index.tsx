...
export default function PrivatePoolVillaPhuketPage({
  partners,
  lastUpdated,
}: Props) {
  ...
  if (isNl) {
    return (
      <PhuketRomanceVillaGuideNl
        owner="private-pool-search"
        primaryUrl={partners.trip_private_pool.partnerUrl}
        secondaryUrl={partners.trip_villa_search.partnerUrl}
      />
    );
  }
  const placementSubId = (placement: string) =>
    `${subId}-pseo-private-pool-villa-phuket-${placement}`;

  const breadcrumbs = [
    { name: "Home", href: "/" },
    {
      name: isNl ? "Private pool villa Phuket" : "Private Pool Villa Phuket",
        ...
        <section className="bg-thailand-blue text-white">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <p className="mt-4 text-sm font-semibold uppercase tracking-wide text-white/75">
              {isNl
                ? "Private pool villa Phuket-gids"
                : "Private pool villa Phuket guide"}
            </p>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <a
                href={withSubId(
                  partners.trip_private_pool.partnerUrl,
                  placementSubId("hero-trip"),
                )}
                target="_blank"
            ...
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-1 text-xs opacity-90">
              <span>
                ✔ 7 {isNl ? "echte privé-zwembaden" : "truly private pools"}
              </span>
              <span>
                ✔ {isNl ? "Wat je werkelijk krijgt" : "What you actually get"}
              </span>
              <span>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          ...
          <section id="comparison">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-2">
              {isNl
                ? "Snelle vergelijking: 7 echte private pool villa-resorts"
                : "Quick comparison: 7 truly private pool villa resorts"}
            </h2>
            <p className="text-gray-600 mb-6">
              {isNl
                ? "Alleen resorts waar elk zwembad exclusief van jou is — geen gedeelde pools."
                : "Only resorts where each pool is exclusively yours — no shared pools."}
            </p>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 text-gray-700">
                  <tr>
                    ...
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Pool-formaat" : "Pool size"}
                    </th>
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Verwarmd?" : "Heated?"}
                    </th>
                    <th className="text-left font-semibold px-4 py-3">
                      {isNl ? "Vanaf" : "From"}
                    </th>
                  ...
                  {[
                    {
                      ...
                      price: "$5,000",
                      best: isNl
                        ? "Privé-estate prestige"
                        : "Private estate prestige",
                    },
                    {
                      name: "Trisara",
                      size: "8×3.5 m infinity",
                      heated: isNl ? "Ja" : "Yes",
                      price: "$2,500",
                      best: isNl ? "Oceanfront privacy" : "Oceanfront privacy",
                    },
                    {
                      name: "Sri Panwa",
                      size: "7×3.5 m",
                      heated: isNl ? "Optioneel" : "Optional",
                      price: "$1,200",
                      best: isNl ? "Cliff-uitzicht" : "Cliff views",
                    },
                    {
                    ...
                    <tr key={i} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer nofollow sponsored"
                          className="font-semibold text-thailand-blue hover:text-thailand-red hover:underline"
                        >
                          {r.name}
                        </a>
                      </td>
            ...
            <p className="mt-3 text-xs text-gray-500">
              {isNl
                ? '"Vanaf"-prijzen zijn 1-bedroom configuraties in laag-hoogseizoen (laat-nov of begin-mei). Hoogseizoen + grotere villa\'s lopen tot $10–15K/nacht.'
                : '"From" prices are 1-bedroom configs in shoulder/start of high season (late Nov / early May). Peak season + larger villas run to $10–15K/night.'}
            </p>
          </section>
          ...
          <section className="rounded-2xl bg-thailand-red/5 border-2 border-thailand-red p-6">
            <span className="inline-block rounded-full bg-thailand-red text-white text-xs font-bold uppercase tracking-wide px-3 py-1 mb-3">
              ⭐ {isNl ? "Wat verwachten" : "What to expect"}
            </span>
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-3">
              {isNl
                ? "Wat krijg je in een echte private pool villa in Phuket?"
                : "What do you get in a real private pool villa in Phuket?"}
            </h2>
            <ul className="space-y-2 text-gray-700 mb-3">
              <li>
                • <strong>{isNl ? "Het zwembad: " : "The pool: "}</strong>
                {isNl
                  ? "5×3 m tot 12×5 m afhankelijk van prijspunt. Volledig binnen je eigen muren of hek. Schoongemaakt tweemaal daags. Verwarmd nov–feb bij Trisara/Amanpuri/Andara."
                  : "5×3 m to 12×5 m depending on price tier. Entirely within your own walls or fence. Cleaned twice daily. Heated Nov–Feb at Trisara/Amanpuri/Andara."}
              </li>
              <li>
                •{" "}
                <strong>
                ...
                {isNl
                  ? "Sun loungers, eettafel buiten, vaak een outdoor sala (paviljoen). Volledig buiten zicht van andere gasten."
                  : "Sun loungers, outdoor dining table, often an outdoor sala (pavilion). Completely out of sight of other guests."}
              </li>
              <li>
                •{" "}
                <strong>
                  {isNl
                    ? "Eigen ingang en toegang: "
                    : "Own entrance and access: "}
                </strong>
                {isNl
                  ? "Geen hotel-gang door, eigen privé-pad of golf-cart-pickup vanaf de lobby."
                  : "No hotel corridor, own private path or golf-cart pickup from the lobby."}
              </li>
              <li>
                •{" "}
                <strong>
                  {isNl
                    ? "Volledig of half ingerichte keuken: "
                    : "Full or partial kitchen: "}
                </strong>
                {isNl
                  ? "Standaard bij Andara, Banyan Tree, The Pavilions. Trisara: kitchenette. Amanpuri: persoonlijke chef op afroep."
                  : "Standard at Andara, Banyan Tree, The Pavilions. Trisara: kitchenette. Amanpuri: personal chef on request."}
              </li>
              <li>
                •{" "}
                <strong>
                  {isNl
                    ? "Villa-attendant of butler: "
                    : "Villa attendant or butler: "}
                </strong>
                {isNl
                  ? "Dedicated boven $1.500/nacht; on-call op $700–1.400/nacht."
                  : "Dedicated above $1,500/night; on-call at $700–1,400/night."}
              </li>
              <li>
                •{" "}
                <strong>
                  {isNl ? "In-villa dining: " : "In-villa dining: "}
                </strong>
                {isNl
                  ? "Ontbijt, lunch, diner aan je eigen poolside set-up. Service charge gewoonlijk 10% bovenop menukosten."
                  : "Breakfast, lunch, dinner at your own poolside setup. Service charge typically 10% on top of menu prices."}
              </li>
            </ul>
          ...
          <section>
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {isNl
                ? "Beste operators voor private pool villa's"
                : "Best operators for private pool villas"}
            </h2>
            <div className="space-y-5">
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
                <h3 className="font-heading text-xl font-bold text-gray-900 mb-2">
                  {isNl
                    ? "$700–1.400/nacht — Value tier"
                    : "$700–1,400/night — Value tier"}
                </h3>
                <p className="text-gray-700 leading-relaxed mb-3">
                  {isNl
                    ? "Banyan Tree (Bang Tao lagoon villa's, $800+), The Pavilions (Cherng Talay adults-only, $700+), Cape Sienna (Kamala met panorama, $600+). Echte privé-zwembaden, 6×3 m tot 8×3,5 m. Niet altijd verwarmd. Butler op afroep, niet dedicated. Sterke keuze voor stellen op 4–7 nacht."
                    : "Banyan Tree (Bang Tao lagoon villas, $800+), The Pavilions (Cherng Talay adults-only, $700+), Cape Sienna (Kamala with panorama, $600+). Genuinely private pools, 6×3 m to 8×3.5 m. Not always heated. Butler on call rather than dedicated. Strong choice for couples on a 4–7 night stay."}
                </p>
                <a
                  href={withSubId(
                    partners.trip_private_pool.partnerUrl,
                    placementSubId("detail-value"),
                  )}
                  target="_blank"
                  rel="noopener noreferrer nofollow sponsored"
                ...
                >
                  {isNl
                    ? "Bekijk value-tier private pool villa's →"
                    : "See value-tier private pool villas →"}
                </a>
              </div>
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
                <h3 className="font-heading text-xl font-bold text-gray-900 mb-2">
                  {isNl
                    ? "$1.500–4.500/nacht — Premium tier"
                    : "$1,500–4,500/night — Premium tier"}
                </h3>
                <p className="text-gray-700 leading-relaxed mb-3">
                  {isNl
                    ? "Trisara (Layan, oceanfront infinity pools), Sri Panwa (Cape Panwa cliff villa's), Andara (Kamala hillside, families). 7×3,5 m tot 8×4 m verwarmde zwembaden. Dedicated villa-attendant inbegrepen, in-villa dining standaard, signature spa's. Sweet spot voor 6–8 daagse romantische trip of multi-gen vacation."
                    : "Trisara (Layan, oceanfront infinity pools), Sri Panwa (Cape Panwa cliff villas), Andara (Kamala hillside, families). 7×3.5 m to 8×4 m heated pools. Dedicated villa attendant included, in-villa dining standard, signature spas. Sweet spot for 6–8 night romantic trip or multi-gen vacation."}
                </p>
                <a
                  href={withSubId(
                    partners.trip_private_pool.partnerUrl,
                    placementSubId("detail-premium"),
                  )}
                  target="_blank"
                  rel="noopener noreferrer nofollow sponsored"
                  className="inline-flex items-center text-sm font-semibold text-thailand-red hover:underline"
                >
                  {isNl
                    ? "Bekijk premium private pool villa's →"
                    : "See premium private pool villas →"}
                </a>
              </div>
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
                <h3 className="font-heading text-xl font-bold text-gray-900 mb-2">
                  {isNl
                    ? "$5.000+/nacht — Ultra-luxe"
                    : "$5,000+/night — Ultra-luxury"}
                </h3>
                <p className="text-gray-700 leading-relaxed mb-3">
                  {isNl
                    ? "Amanpuri private estates (2-tot-9-bedroom met eigen chef, butler, masseuse). 12×5 m hoofdzwembad plus secundaire plunge pool. Het meest persoonlijke service-niveau in Phuket — alleen op de Mandarin Oriental private estates op Bangkok of Four Seasons Koh Samui in dezelfde range."
                    : "Amanpuri private estates (2-to-9-bedroom with own chef, butler, masseuse). 12×5 m main pool plus secondary plunge pool. The most personalised service tier in Phuket — only Mandarin Oriental private estates in Bangkok or Four Seasons Koh Samui in the same range."}
                </p>
                <a
                  href={withSubId(
                    partners.trip_private_pool.partnerUrl,
                    placementSubId("detail-ultra"),
                  )}
                  target="_blank"
                  rel="noopener noreferrer nofollow sponsored"
                  className="inline-flex items-center text-sm font-semibold text-thailand-red hover:underline"
                >
                  {isNl
                    ? "Bekijk ultra-luxe private estates →"
                    : "See ultra-luxury private estates →"}
                </a>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl bg-amber-50 border border-amber-200 p-6">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {isNl
                ? "Boekingstips: zorg dat het écht privé is"
                : "Booking tips: make sure it's actually private"}
            </h2>
            <ul className="space-y-3 text-gray-800">
              <li className="flex gap-3">
                ...
                <span>
                  <strong>
                    {isNl
                      ? 'Eis "exclusive use" in de bevestiging'
                      : 'Demand "exclusive use" in the confirmation'}
                  </strong>
                  {isNl
                    ? '. Niet alleen "private pool" — sommige resorts verkopen "private" pool maar 4 villa\'s delen toegang. Vraag schriftelijk: "pool exclusively for our villa, no shared access".'
              ...
              <li className="flex gap-3">
                ...
                <span>
                  <strong>
                    {isNl ? "Vraag pool-formaat in m²" : "Ask pool size in m²"}
                  </strong>
                  {isNl
                    ? '. "Pool villa" kan plunge pool van 3×2 m betekenen. Voor échte zwemmen: minimum 5 m lang.'
                    : '. "Pool villa" can mean 3×2 m plunge pool. For actual swimming: minimum 5 m long.'}
                </span>
              </li>
              <li className="flex gap-3">
                <span className="text-amber-700 font-bold">→</span>
                <span>
                  <strong>
                    {isNl ? "Verwarming nov–feb" : "Heating Nov–Feb"}
                  </strong>
                  {isNl
                    ? ". Standaard bij Trisara, Amanpuri, Andara. Optioneel/niet bij The Pavilions, Cape Sienna. Bij <26°C ochtend-water voelt onverwarmd koud."
                    : ". Standard at Trisara, Amanpuri, Andara. Optional/none at The Pavilions, Cape Sienna. Below 26°C morning water, unheated feels cold."}
                </span>
              </li>
              <li className="flex gap-3">
                <span className="text-amber-700 font-bold">→</span>
                <span>
                  <strong>
                    {isNl
                      ? "Pool-fence voor peuters"
                      : "Pool fence for toddlers"}
                  </strong>
                  {isNl
                    ? ". Add-on $50/nacht bij Banyan Tree, Andara, Sri Panwa. Mesh-hek rond bad — vraag als je kids onder 4 hebt."
                    : ". Add-on $50/night at Banyan Tree, Andara, Sri Panwa. Mesh fence around the pool — request if you have kids under 4."}
                </span>
              </li>
              <li className="flex gap-3">
                <span className="text-amber-700 font-bold">→</span>
                <span>
                  <strong>
                    {isNl
                      ? "Vergelijk Trip.com vs direct booking"
                      : "Compare Trip.com vs direct booking"}
                  </strong>
                  {isNl
                    ? ". Direct boeken: extra perks (transfer, ontbijt, spa-credit). Trip.com: vaak 10–25% lagere kale prijs. Soms wint direct, soms Trip.com — altijd beide checken."
                    : ". Direct booking: extra perks (transfer, breakfast, spa credit). Trip.com: often 10–25% lower base price. Sometimes direct wins, sometimes Trip.com — always check both."}
                </span>
              </li>
              <li className="flex gap-3">
                <span className="text-amber-700 font-bold">→</span>
                <span>
                  <strong>
                    {isNl
                      ? "Service charge + btw nakijken"
                      : "Check service charge + VAT"}
                  </strong>
                  {isNl
                    ? '. 10% service + 7% btw is standaard, vaak NIET in de "from" prijs. Reken altijd 17% extra.'
                    : '. 10% service + 7% VAT is standard, often NOT in the "from" price. Always add 17%.'}
                </span>
              </li>
              <li className="flex gap-3">
                <span className="text-amber-700 font-bold">→</span>
                <span>
                  <strong>
                    {isNl
                      ...
                      : "Book the largest configuration"}
                  </strong>
                  {isNl
                    ? ". Een 3-bedroom villa is vaak slechts 60% duurder dan 1-bedroom — voor families/groepen significant goedkoper per persoon."
                    : ". A 3-bedroom is often only 60% more than 1-bedroom — significantly cheaper per person for families/groups."}
                </span>
              </li>
            </ul>
          </section>
          ...
          <section className="rounded-2xl bg-thailand-blue/10 p-6">
            ...
            <p className="mt-2 text-gray-700">
              {isNl
                ? "Pool villa is geboekt — werk de rest af:"
                : "Pool villa booked — wrap up the rest:"}
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <Link
                href="/phuket-luxury-villas/family/"
              ...
              >
                {isNl
                  ? "👨‍👩‍👧 Family pool villa's (4+ slaapkamers)"
                  : "👨‍👩‍👧 Family pool villas (4+ bedrooms)"}
              </Link>
              <Link
                href="/phuket-luxury-villas/oceanfront/"
                className="rounded-full bg-white text-thailand-blue border border-thailand-blue px-5 py-2 text-sm font-semibold hover:bg-thailand-blue hover:text-white"
              >
                {isNl
                  ? "🌊 Oceanfront pool villa's"
              ...
              <Link
                ...
                className="rounded-full bg-white text-thailand-blue border border-thailand-blue px-5 py-2 text-sm font-semibold hover:bg-thailand-blue hover:text-white"
              >
                {isNl ? "🚗 Auto huren in Phuket" : "🚗 Car rental Phuket"}
              </Link>
              <a
