...
const SPOKE_LABELS: Record<SpokeSlug, { en: string; nl: string }> = {
  ...
  "all-inclusive": {
    en: "All-inclusive honeymoon",
    nl: "All-inclusive honeymoon",
  },
};

export default function PhuketHoneymoonSpokePage({
  spoke,
  primaryUrl,
  secondaryUrl,
  partners,
  ...
  if (isNl) {
    const owners: Record<SpokeSlug, PhuketRomanceVillaOwner> = {
      "private-pool-villas": "honeymoon-pool",
      "all-inclusive": "honeymoon-inclusive",
    };
    return (
  ...
  const placement = (slot: string) =>
    `${subId}-pseo-phuket-honeymoon-${spoke}-${slot}`;

  const breadcrumbs = [
    { name: "Home", href: "/" },
    {
      name: isNl ? "Phuket honeymoon" : "Phuket Honeymoon",
      href: "/phuket-honeymoon/",
    },
    {
      name: isNl ? SPOKE_LABELS[spoke].nl : SPOKE_LABELS[spoke].en,
      href: `/phuket-honeymoon/${spoke}/`,
    },
  ];
  ...
  if (spoke === "private-pool-villas") {
    ...
    descEn =
      "Looking for a private pool villa for your Phuket honeymoon? Compare 7 romantic picks (Trisara, Sri Panwa, Anantara) from $700 to $4,500/night.";
    descNl =
      "Op zoek naar een privé pool-villa voor je Phuket-honeymoon? Vergelijk 7 romantische keuzes (Trisara, Sri Panwa, Anantara) $700–4.500/nacht.";
    ...
    descEn =
      "Looking for an all-inclusive Phuket honeymoon? Compare 6 packages (Banyan Tree, Anantara, Centara) — what's in, what's extra and real costs.";
    descNl =
      "Op zoek naar all-inclusive Phuket honeymoon? Vergelijk 6 pakketten (Banyan Tree, Anantara, Centara) — wat zit erin, wat is extra en echte kosten.";
  ...
  const heroStats = (() => {
    if (spoke === "private-pool-villas") {
      return [
        ...
        {
          label: isNl ? "Honeymoon-perks" : "Honeymoon perks",
          value: isNl ? "Standaard" : "Standard",
        },
      ];
    }
    return [
  ...
  const tableRows = (() => {
    if (spoke === "private-pool-villas") {
      return [
        {
          label: "Trisara (Layan)",
          spec: isNl ? "39 villa's, beachfront" : "39 villas, beachfront",
          price: "$1,200–3,500",
          best: isNl ? "Iconic luxe + privacy" : "Iconic luxury + privacy",
        },
        {
          label: "Amanpuri (Pansea)",
          spec: isNl ? "40 villa's, privé baai" : "40 villas, private bay",
          price: "$1,500–4,500",
          best: isNl ? "Ultra-luxe rust" : "Ultra-luxury serenity",
        },
        {
          label: "Sri Panwa (Cape Panwa)",
          spec: isNl ? "Cliffside pool-villa" : "Cliffside pool villa",
          price: "$700–2,500",
          best: isNl ? "Sunset uitzicht" : "Sunset view",
        },
        {
          label: "Anantara Mai Khao",
          spec: isNl
            ? "Garden + beach pool-villa"
            : "Garden + beach pool villa",
          price: "$450–1,200",
          best: isNl ? "Rust + dropdown prijs" : "Quiet + great value",
        },
        {
          label: "Banyan Tree Bang Tao",
          spec: isNl ? "Lagune-villa + spa" : "Lagoon villa + spa",
          price: "$500–1,800",
          best: isNl ? "Spa-honeymoon" : "Spa-focused honeymoon",
        },
        {
          label: "The Pavilions Phuket",
          spec: isNl ? "Cherngtalay heuveltop" : "Cherngtalay hilltop",
          price: "$350–950",
          best: isNl ? "Mid-budget intiem" : "Mid-budget intimate",
        },
        {
          label: "Andara Resort (Kamala)",
          spec: isNl ? "5–9 SK villa's" : "5–9 BR villas",
          price: "$800–2,800",
          best: isNl ? "Multi-day takeover" : "Multi-day takeover",
        },
      ];
    }
    return [
      {
        label: "Centara Grand Beach Resort",
        spec: isNl ? "Karon Beach, family-luxe" : "Karon Beach, family luxury",
        price: "$280–550/n",
        best: isNl ? "Beste pakket-value" : "Best package value",
      },
      {
        label: "Outrigger Laguna Phuket",
        spec: isNl ? "Lagune Bang Tao" : "Lagoon Bang Tao",
        price: "$320–680/n",
        best: isNl ? "Resort-faciliteiten" : "Full resort facilities",
      },
      {
        label: "Anantara Mai Khao",
        spec: isNl ? "Pool-villa al-in optie" : "Pool villa AI option",
        price: "$650–1,400/n",
        best: isNl ? "Luxe AI met privacy" : "Luxury AI + privacy",
      },
      {
        label: "Banyan Tree Bang Tao",
        spec: isNl ? "Spa + dining inclusief" : "Spa + dining included",
        price: "$700–1,900/n",
        best: isNl ? "Spa-rich honeymoon" : "Spa-rich honeymoon",
      },
      {
        label: "JW Marriott Mai Khao",
        spec: isNl ? "Family-resort, 11km strand" : "Family resort, 11km beach",
        price: "$420–820/n",
        best: isNl ? "Brede gasten-leeftijd" : "Wide guest age range",
      },
      {
        label: "Holiday Inn Mai Khao",
        spec: isNl ? "Mid-range AI mogelijk" : "Mid-range AI option",
        price: "$220–380/n",
        best: isNl ? "Onder-$300 budget" : "Under-$300 budget",
      },
    ];
  })();
  ...
  const sections = (() => {
    if (spoke === "private-pool-villas") {
      return [
        {
          title: isNl
            ? "Wat onderscheidt een echte honeymoon-villa?"
            : "What makes a real honeymoon villa",
          body: isNl
        ...
        {
          title: isNl
            ? "Honeymoon-perks die élke luxe-villa biedt"
            : "Honeymoon perks every luxury villa offers",
          body: isNl
        ...
        {
          title: isNl
            ? "Pool-villa vs hotel-suite: cost-benefit"
            : "Pool villa vs hotel suite: cost vs benefit",
          body: isNl
    ...
    return [
      {
        title: isNl
          ? "Wat 'all-inclusive' in Phuket écht betekent"
          : "What 'all-inclusive' actually means in Phuket",
        body: isNl
      ...
      {
        title: isNl
          ? "Rekenen: AI vs los — wanneer wint AI?"
          : "Math: AI vs separate — when AI wins",
        body: isNl
      ...
      {
        title: isNl
          ? "Beste tier-AI voor honeymoon (niet familie-AI)"
          : "Best honeymoon-tier AI (not family AI)",
        body: isNl
  ...
  const tips = (() => {
    if (spoke === "private-pool-villas") {
      return [
        {
          strong: isNl
            ? "Vermeld honeymoon bij boeking ÉN check-in"
            : "Mention honeymoon at booking AND check-in",
          body: isNl
            ? "— online-bookers vergeten dit, het is essentiële stap voor gratis upgrades, champagne, bloemen."
            : "— online bookers forget this, the essential step for free upgrades, champagne, flowers.",
        },
        {
          strong: isNl
            ? "Boek 4–6 maanden vooruit voor hoogseizoen"
            : "Book 4–6 months ahead for high season",
          body: isNl
            ? "— Trisara, Amanpuri, Sri Panwa zit voor nov–feb 90+ dagen vooruit op slot."
            : "— Trisara, Amanpuri, Sri Panwa book out 90+ days ahead Nov–Feb.",
        },
        {
          strong: isNl
            ? "Vraag privé chef-optie aan"
            : "Ask about private chef",
          body: isNl
            ? "— bij premium-villa's standaard. $80–250 voor een 4-gangen diner-thuis. Romantischer dan restaurant + photo-worthy."
            : "— standard at premium villas. $80–250 for a 4-course in-villa dinner. More romantic than restaurant + photo-worthy.",
        },
        {
          strong: isNl
            ? "Pool-positie checken (privacy)"
            : "Check pool position (privacy)",
          body: isNl
            ? "— sommige villa's hebben overlap met buren. Vraag floorplan + photo voor je boekt — Trisara villa-types verschillen sterk."
            : "— some villas overlap with neighbours. Ask for the floorplan + photo before booking — Trisara villa types vary widely.",
        },
        {
          strong: isNl ? "Bath-products check" : "Check bath products",
          body: isNl
            ? "— top-villa's leveren Aman, Le Labo, Diptyque amenities; mid-range geeft generieke. Klein detail dat vakantie-fotos verbetert."
            : "— top villas stock Aman, Le Labo, Diptyque amenities; mid-range stocks generic. Small detail that elevates vacation photos.",
        },
        {
          strong: isNl ? "Late check-out vragen" : "Ask for late check-out",
          body: isNl
            ? "— honeymooners krijgen meestal 14:00 of 16:00 i.p.v. 11:00 standaard. Gratis bij vermelding."
            : "— honeymooners usually get 14:00 or 16:00 instead of 11:00 standard. Free with mention.",
        },
        {
          strong: isNl
            ? "Reisverzekering met luxe-tier"
            : "Travel insurance with luxury tier",
          body: isNl
            ? "— $4.000+ accommodatie heeft luxe-niveau insurance nodig. Premium polissen kosten 5–7% van trip-budget."
            : "— $4,000+ accommodation needs luxury-tier insurance. Premium policies cost 5–7% of trip budget.",
        },
      ];
    }
    return [
      {
        strong: isNl
          ? "Reken eigen scenario, geen generiek advies"
          : "Run your own numbers, not generic advice",
        body: isNl
          ? "— AI is geen win voor lichte eters of restaurant-fans. Schat je eigen eten/drinks-uitgaven los voor je AI accepteert."
          : "— AI is not a win for light eaters or restaurant fans. Estimate your own food/drink spend separately before agreeing to AI.",
      },
      {
        strong: isNl
          ? "Vraag wat ÉCHT inbegrepen is"
          : "Ask what is REALLY included",
        body: isNl
          ? '— "all-inclusive" varieert per resort. Vraag itemised PDF: alcohol-tier, restaurants, spa, watersport, dagtochten.'
          : '— "all-inclusive" varies by resort. Ask for itemised PDF: alcohol tier, restaurants, spa, watersports, day tours.',
      },
      {
        strong: isNl
          ? "Honeymoon-tier resorts kiezen"
          : "Pick honeymoon-tier resorts",
        body: isNl
          ? ": Anantara, Banyan Tree, Outrigger boven Holiday Inn of Centara Family. Adult-preferred pool maakt verschil."
          : ": Anantara, Banyan Tree, Outrigger over Holiday Inn or Centara Family. Adult-preferred pool makes the difference.",
      },
      {
        strong: isNl
          ? "Reserveer à-la-carte op aankomst"
          : "Reserve à-la-carte on arrival",
        body: isNl
          ? "— populaire restaurants in hoog-seizoen vol. AI dekt het maar je moet plek hebben — boek alle 7 nachten in eerste 24u."
          : "— popular restaurants fill in high season. AI covers it but you need a table — book all 7 nights in the first 24h.",
      },
      {
        strong: isNl
          ? "Min 5 nachten voor AI break-even"
          : "Min 5 nights for AI break-even",
        body: isNl
          ? "— eronder verlies je via early-check-out + late-check-in onbenutte maaltijden. 7 nachten is sweet-spot."
          : "— below that you lose via early check-out + late check-in unused meals. 7 nights is the sweet spot.",
      },
      {
        strong: isNl
          ? "Vraag honeymoon-room-upgrade"
          : "Ask for honeymoon room upgrade",
        body: isNl
          ? "— ja, ook bij AI. Free upgrade naar volgende kamer-tier op basis van availability bij vermelding."
          : "— yes, even on AI. Free upgrade to next room tier subject to availability with mention.",
      },
      {
        strong: isNl
          ? "Plan 1 off-resort uitstapje"
          : "Plan 1 off-resort outing",
        body: isNl
          ? "— Phuket Old Town diner of sunset bij Promthep Cape. Tegen AI-burnout op dag 4–5."
          : "— Phuket Old Town dinner or sunset at Promthep Cape. Antidote to AI burnout on day 4–5.",
      },
    ];
  })();
  ...
  const faqEn = (() => {
    ...
    return [
      {
        q: "Is an all-inclusive Phuket honeymoon worth it?",
        a: "Math depends on your eating + drinking habits. Example: 7 nights, 2 guests at Centara — AI package $2,730, separate booking estimated $4,060 for the same room + 3 meals + 5 daily cocktails. AI saves $1,330 (33%) IF you eat 3 hotel meals/day + drink alcohol regularly. For light eaters or couples who want off-property dining: separate booking wins. Run your own numbers before agreeing to AI — never assume the brochure quote is universal.",
      },
      {
        q: 'What does "all-inclusive" actually include in Phuket?',
  ...
  const faqNl = (() => {
    if (spoke === "private-pool-villas") {
      return [
        ...
        {
          q: "Welke Phuket pool-villa heeft beste honeymoon-perks?",
    ...
    return [
      ...
      {
        q: "Hoeveel dagen minimaal voor zinnig all-inclusive honeymoon?",
        a: "5 nachten min, 7 ideaal. Onder 5 nachten verlies je waarde via early check-out + late check-in onbenutte maaltijden. Bij 7 nachten bespaart AI 25–35% vs los als je het volledig benut. Bij 10+ nachten loopt besparing 35–45% op. Voor 3–4 nachten wint los-boeken bijna altijd omdat dagelijkse AI-tarief slecht amortiseert over weinig dagen.",
      },
      {
        q: "Kunnen we het AI-pakket onderhandelen?",
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              >
                {spoke === "private-pool-villas"
                  ? isNl
                    ? "Bekijk pool-villa's op Trip.com"
                    : "See pool villas on Trip.com"
                  : isNl
                    ? "Bekijk all-inclusive op Trip.com"
                    : "See all-inclusive on Trip.com"}{" "}
                →
              </a>
              <a
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          ...
          <section id="comparison">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-2">
              {spoke === "private-pool-villas"
                ? isNl
                  ? "Pool-villa-honeymoons vergeleken"
                  : "Pool villa honeymoons compared"
                : isNl
                  ? "All-inclusive honeymoon-resorts vergeleken"
                  : "All-inclusive honeymoon resorts compared"}
            </h2>
            ...
            <p className="mt-3 text-xs text-gray-500">
              {isNl
                ? "Prijzen zijn 2026 hoogseizoen-tarieven (nov–feb) per nacht voor 2 gasten. Buiten hoogseizoen 25–35% lager. Honeymoon-perks gratis bij vermelding bij boeking."
                : "Prices are 2026 high-season rates (Nov–Feb) per night for 2 guests. Outside high season 25–35% cheaper. Honeymoon perks free with mention at booking."}
            </p>
          </section>
          ...
          <section className="rounded-2xl bg-thailand-red/5 border-2 border-thailand-red p-6">
            ...
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-3">
              {spoke === "private-pool-villas"
                ? isNl
                  ? "Trisara — beachfront pool-villa, volle honeymoon-perks"
                  : "Trisara — beachfront pool villa, full honeymoon perks"
                : isNl
                  ? "Anantara Mai Khao Pool Villa AI — luxe + privacy"
                  : "Anantara Mai Khao Pool Villa AI — luxury + privacy"}
            </h2>
          ...
          <section className="rounded-2xl bg-amber-50 border border-amber-200 p-6">
            <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
              {spoke === "private-pool-villas"
                ? isNl
                  ? "Boekingstips pool-villa honeymoon"
                  : "Booking tips for pool villa honeymoon"
                : isNl
                  ? "Boekingstips all-inclusive honeymoon"
                  : "Booking tips for all-inclusive honeymoon"}
            </h2>
          ...
          {siblings.length > 0 && (
            <section>
              <h2 className="font-heading text-2xl font-bold text-gray-900 mb-4">
                {isNl
                  ? "Andere honeymoon-opties in Phuket"
                  : "Other honeymoon options in Phuket"}
              </h2>
              <div className="grid sm:grid-cols-2 gap-3">
                {siblings.map((s) => (
                  <Link
                    key={s}
                    href={`/phuket-honeymoon/${s}/`}
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {s === "private-pool-villas" &&
                        (isNl
                          ? "7 pool-villa picks, $700–4.500/nacht"
                          : "7 pool villa picks, $700–4,500/night")}
                      {s === "all-inclusive" &&
                        (isNl
                          ? "6 AI honeymoon-resorts vergeleken"
                          : "6 AI honeymoon resorts compared")}
                    </p>
          ...
          <section className="rounded-2xl bg-thailand-blue/10 p-6">
            <h2 className="font-heading text-xl font-bold text-gray-900">
              {isNl
                ? "Plan je hele Phuket honeymoon"
                : "Plan your full Phuket honeymoon"}
            </h2>
            <p className="mt-2 text-gray-700">
              {isNl
                ? "Resort gekozen — werk de rest af:"
                : "Resort picked — wrap up the rest:"}
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                {isNl
                  ? "💕 Alle 8 honeymoon-resorts vergelijken"
                  : "💕 Compare all 8 honeymoon resorts"}
              </Link>
              <Link
              ...
              >
                {isNl ? "💒 Trouwlocaties Phuket" : "💒 Wedding venues Phuket"}
              </Link>
              <Link
              ...
              >
                {isNl ? "🏨 Alle Phuket hotels" : "🏨 All Phuket hotels"}
              </Link>
              <Link
                href="/yacht-charter-phuket/"
                className="rounded-full bg-white text-thailand-blue border border-thailand-blue px-5 py-2 text-sm font-semibold hover:bg-thailand-blue hover:text-white"
              >
                {isNl ? "⛵ Yacht charter sunset" : "⛵ Yacht charter sunset"}
              </Link>
              <Link
              ...
              <a
                href={withSubId(KLOOK_GENERIC, placement("mesh-klook"))}
                target="_blank"
                rel="noopener noreferrer nofollow sponsored"
                className="rounded-full bg-thailand-red text-white px-5 py-2 text-sm font-semibold hover:bg-red-700"
              >
                {isNl ? "🎟️ Couples-activiteiten" : "🎟️ Couples activities"}
              </a>
            </div>
          </section>
